...
def solve_pdp(args: Namespace, config: PDPConfig) -> None:
    ...
    # solver.plot_solution(args.plot_output)
...
def solve_epdp(args: Namespace, config: EPDPConfig) -> None:
    ...
    solver.solve_model(settings=config.solver_settings,
                       log_search=True, initial_solution_filepath=args.initial_solution_path)
...
def main() -> None:
    ...
    epdp_parser.add_argument(
        "--initial_solution_path",
        default=None,
        type=str,
        help="filepath for initial solution"
    )
